...
const Senses = () => {
  ...
  const [currentSenseId, setCurrentSenseId] = useState(null);
  ...
  const handleSubmit = async () => {
    try {
      ...
      let response;
      if (isEditing) {
        response = await fetch(`http://localhost:3001/updateSense/${currentSenseId}`, {
          method: 'PUT',
          headers: {
        ...
        response = await fetch('http://localhost:3001/createSense', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
      ...
      if (isEditing) {
        setSenses(senses.map(sense => (sense._id === currentSenseId ? { ...sense, title } : sense)));
      } else {
        setSenses([...senses, result]);
      }
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(`http://localhost:3001/deleteSense/${id}`, {
        method: 'DELETE',
      });
      if (!response.ok) {
        throw new Error('Network response was not ok');
      }
      setSenses(senses.filter(sense => sense._id !== id));
    } catch (error) {
      console.error('Error:', error);
    }
  };

  const handleEdit = (sense) => {
    setInputValue(sense.title);
    setIsEditing(true);
    setCurrentSenseId(sense._id);
  };
  ...
  return (
    <div className='mx-5'>
      <div className='d-flex justify-content-between align-items-center'>
        <h1>Senses</h1>
        <button type="button" style={{backgroundColor:"#ffe8b5",color:"#191f45"}} className="btn btn-primary" data-bs-toggle="modal" data-bs-target="#staticBackdrop">
          Add Sense
        </button>
      </div>
      <div className='' >
        ...
        <div className="modal-dialog">
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title" id="staticBackdropLabel" style={{color: "black"}}>{isEditing ? 'Update Sense' : 'Add Sense'}</h5>
            ...
            <div className="modal-footer">
              ...
              <button type="button" onClick={handleSubmit} style={{backgroundColor:"#ffe8b5",color:"#191f45"}} className="btn btn-primary">{isEditing ? 'Update' : 'Add'}</button>
            </div>
          </div>
        </div>
      </div>
